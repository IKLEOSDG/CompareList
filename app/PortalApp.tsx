...
function MobileNav({ section }: { section: Section }) {
  return <nav className="yj-mobile-nav"><a className={section === "home" ? "active" : ""} href="#home">首页</a><a className={section === "design" ? "active" : ""} href="#design">图纸</a><a className={section === "materials" ? "active" : ""} href="#materials">选材</a><a className={section === "budget" ? "active" : ""} href="#budget">预算</a><a className={section === "documents" ? "active" : ""} href="#documents">资料</a></nav>;
}
...
function DesignPage() {
  ...
  return <div className="yj-page"><PageHead eyebrow="DESIGN DRAWINGS" title="设计图纸" lead="图纸直接展示；点击任意图纸可放大查看细节与尺寸。" /><div className="yj-filter-row">{groups.map((item) => <button className={group === item ? "active" : ""} onClick={() => setGroup(item)} key={item}>{item}</button>)}</div><section className="yj-drawing-grid">{list.map((item) => <button key={item.name} onClick={() => setActive(item)}><div><img src={item.image} alt={item.name} /></div><span>{item.group}</span><h3>{item.name}</h3><p>{item.note}</p></button>)}</section><div className="yj-drawing-note"><b>施工边界</b><span>长城铺贴图和卫生间立面均为排版参考，不代替现场复尺、损耗计算、收口节点及最终施工确认。</span></div>{active && <div className="yj-lightbox" onClick={() => setActive(null)}><button onClick={() => setActive(null)}>关闭 ×</button><img src={active.image} alt={active.name} /><footer><b>{active.name}</b><span>{active.note}</span></footer></div>}</div>;
}
